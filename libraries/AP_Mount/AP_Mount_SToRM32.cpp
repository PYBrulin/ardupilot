...
void AP_Mount_SToRM32::update()
{
    ...
    // and add stabilization relative to body frame if the mount cannot stabilize its axis by itself
    // _angle_bf_output_deg act as the angle compensation in case the gimbal cannot control its own axis
    if (resend_now || ((AP_HAL::millis() - _last_send) > AP_MOUNT_STORM32_RESEND_MS)) {
        send_do_mount_control(ToDeg(_angle_ef_target_rad.y) + _angle_bf_output_deg.y,
                              ToDeg(_angle_ef_target_rad.x) + _angle_bf_output_deg.x,
